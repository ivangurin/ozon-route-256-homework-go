...
func (a *api) AddItem() func(w http.ResponseWriter, r *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		logger.Infof(r.Context(), "handleAddItem: start handle request: %s", r.RequestURI)
		defer logger.Infof(r.Context(), "handleAddItem: finish handle request: %s", r.RequestURI)

		req, err := toAddItemRequest(r)
		if err != nil {
			logger.Errorf(r.Context(), "handleAddItem: request is not valid: %v", err)
		...
		if err != nil {
			logger.Errorf(r.Context(), "handleAddItem failed to add item: %v", err)
...
func toAddItemRequest(r *http.Request) (*AddItemRequest, error) {
	...
	if err != nil {
		logger.Errorf(r.Context(), "handleAddItem: failed to unmarshal body json: %v", err)
		return nil, err
	}
